...
if Sys.iswindows()
    ...
    const INVALID_OS_HANDLE = WindowsRawSocket(Ptr{Void}(-1))
else
...
function PipeEndpoint()
    ...
    err = ccall(:uv_pipe_init, Cint, (Ptr{Void}, Ptr{Void}, Cint), eventloop(), pipe.handle, 0)
...
function init_stdio(handle::Ptr{Void})
    ...
    if t == UV_FILE
        fd = ccall(:jl_uv_file_handle, OS_HANDLE, (Ptr{Void},), handle)
...
function open_pipe!(p::PipeEndpoint, handle::OS_HANDLE, readable::Bool, writable::Bool)
    ...
    err = ccall(:jl_pipe_open, Int32, (Ptr{Void}, OS_HANDLE, Cint, Cint), p.handle, handle, readable, writable)
